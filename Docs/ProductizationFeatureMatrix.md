...
| Thermal / fan / acoustics | Power / Thermal category. | `ModernUiPowerDataLib` now; possible future thermal sensor provider if read-only and portable. | Show thermal state/fan capability summary when safe data exists. | Fan curves, pump headers, thermal trip points, acoustic profiles stay native EC/BMC/Super I/O/platform HII. |
