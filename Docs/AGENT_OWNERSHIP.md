...
| `ModernSetupAppProvider.c` | App-private read-only provider snapshot and fallback normalization | Keep provider LibraryClass calls centralized here so Dashboard/pages consume normalized summaries without parsing setup data or duplicating fallback policy |
