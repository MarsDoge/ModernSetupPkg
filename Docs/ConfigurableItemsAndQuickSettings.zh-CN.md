...
并调研 IBV 与信创产品 BIOS 中的高频可配置项，把每一类绑定到一个受控的处理档位。
...
| **B —— 策划式快捷设置深链** | 定位已知高频 HII 问题，分组在现代页呈现；激活时 `SendForm()` 跳进归属 formset/form/问题。**编辑仍原生；ModernDisplayEngine 渲染。** | 原生 FormBrowser + 平台 ConfigAccess | Secure Boot、TPM 使能、VT-d/IOMMU、SR-IOV、Above-4G、ReBAR、SATA 模式、主显选择、WoL、掉电恢复、Fast Boot、TCM/国密 |
...
| 域 | 高频项 | IBV(AMI/Insyde/Phoenix/Byosoft) | 信创(兆芯/海光/飞腾/龙芯/鲲鹏) | 频率 | 档位 |
| --- | --- | --- | --- | --- | --- |
| 启动 | 启动顺序、Fast Boot、CSM/Legacy(x86)、PXE/HTTP boot、超时 | 全有 | 多有(龙芯/飞腾无 CSM) | 高 | B/C |
| 安全 | Secure Boot、TPM/PTT/fTPM 使能、清 TPM、密码 | 全有 | **+ TCM/国密(SM2/3/4)**、国密安全启动证书 | 高 | B/C |
| 虚拟化/隔离 | VT-x/SVM、**VT-d/IOMMU/SMMU**、**SR-IOV**、ACS/ARI/PASID | 全有 | 海光/鲲鹏服务器；飞腾 SMMU | 中-高 | **B/C** |
| PCIe 资源 | **Above-4G、ReBAR、ASPM、链路速率、bifurcation、hot-plug** | 全有 | 服务器侧 | 中-高 | B/C |
| CPU | SMT、C-states、Turbo/Boost、P-state/CPPC、核数开关 | 全有 | 海光/鲲鹏；部分龙芯/飞腾 | 中 | C |
...
| 管理(服务器) | BMC 网络(DHCP/静态)、IPMI over LAN、Redfish 使能 | 服务器 | 海光/鲲鹏服务器 | 中 | C |
| 信创专属 | **国密可信(TCM)、国产化/兼容模式、内核完整性度量** | — | 常见(麒麟/UOS 生态) | 中 | B/C |
...
2. **分组**（安全 / 虚拟化 / PCIe / 电源 / 启动 / 信创）：每项渲染为现代行；能只读读到当前值时
   显示当前值，否则显示"配置 ›"。
...
- AMI Aptio / InsydeH2O / Phoenix SecureCore / 南京百敖 ByoCore 设置参考（仅视觉/信息架构；
  见 IbvAndPlatformSetupSurvey）。
- 信创平台参考：龙芯/飞腾/兆芯/海光/鲲鹏 UEFI 设置；麒麟/UOS 可信计算（TCM/国密）文档。
